...
document.querySelectorAll('.color-option').forEach(option => {
  ...
  if (createTeamBtn) createTeamBtn.addEventListener('click', showCreateTeamForm);
  if (joinTeamBtn) joinTeamBtn.addEventListener('click', showJoinTeamForm);
  if (confirmCreateTeam) confirmCreateTeam.addEventListener('click', createTeam);
  if (confirmJoinTeam) confirmJoinTeam.addEventListener('click', joinTeam);
...
async function switchTab(tabName) {
  ...
  // Load team data when switching to Teams tab
  if (tabName === 'teams' && teamService) {
...
async function setActiveTeam(teamCode) {
  try {
    ...
    // Toggle: if clicking the active team, deactivate it
    if (currentTeam && currentTeam.code === teamCode) {
      // Deactivate team
      await chrome.storage.local.remove('currentTeamCode');
      currentTeamCode = null;
      
      // Clear bubbles from page
      if (tab && tab.id) {
        chrome.tabs.sendMessage(tab.id, { action: 'clearTeamBubbles' });
      }
      ...
      // Load team bubbles on page
      if (tab && tab.id) {
        chrome.tabs.sendMessage(tab.id, { 
          action: 'loadTeamBubbles',
          teamCode: teamCode
        });
      }
      
      console.log('Team activated:', teamCode);
    }
